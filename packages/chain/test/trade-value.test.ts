...
describe("trade valuation", () => {
  test("a buy spends, so it is positive", () => {
    ...
    expect(tradeValueUsd(-500_000_000n, null, null)).toBeCloseTo(500, 9);
  });

  test("a sell returns, so it is negative", () => {
    expect(tradeValueUsd(600_000_000n, null, null)).toBeCloseTo(-600, 9);
  });

  test("falls back to the SOL leg, which most routes actually use", () => {
    // Spent 2 SOL at $200.
    expect(tradeValueUsd(null, -2_000_000_000n, 200)).toBeCloseTo(400, 9);
  });
  ...
  test("prefers the stablecoin leg when both moved", () => {
    ...
    expect(tradeValueUsd(-500_000_000n, -2_000_000_000n, 200)).toBeCloseTo(500, 9);
  });
  ...
  test("a buy and a later sell of the same shares do not net to zero", () => {
    ...
    const buy = tradeValueUsd(-1_000_000_000n, null, null);
    const sell = tradeValueUsd(1_200_000_000n, null, null);
  ...
  test("reports no cost when neither cash leg is present", () => {
    ...
    expect(tradeValueUsd(null, null, 200)).toBeNull();
    expect(tradeValueUsd(0n, 0n, 200)).toBeNull();
  });

  test("reports no cost when the SOL price is unavailable", () => {
    expect(tradeValueUsd(null, -2_000_000_000n, null)).toBeNull();
  });
